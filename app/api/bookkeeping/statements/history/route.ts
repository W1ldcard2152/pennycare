...
// GET /api/bookkeeping/statements/history - Get all import batches with filters
export async function GET(request: NextRequest) {
  try {
    ...
    // Build where clause - exclude CC imports (batches starting with "cc-")
    const where: {
      companyId: string;
      sourceAccountId?: string;
      importedAt?: { gte?: Date; lte?: Date };
      NOT?: { importBatch: { startsWith: string } };
    } = {
      companyId: companyId!,
      NOT: { importBatch: { startsWith: 'cc-' } },
    };
    ...
    // Get all imports matching filters (bank/statement imports only, not CC)
